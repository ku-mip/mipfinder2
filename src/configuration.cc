...
namespace mipfinder
{
	void Parameters::insert(const Parameter& parameter, const Value& value)
	{
		params_.emplace(parameter, value);
	}

	const Parameters::Value& Parameters::operator[](const Parameter& parameter) const
	{
	...
	const Parameters& mipfinder::Configuration::operator[](const Header& header) const
	{
		return config_.contains(header) ? config_.at(header) : Parameters{};
	}
}
...
std::size_t mipfinder::Configuration::size() const noexcept
{
	return config_.size();
}

std::size_t mipfinder::Parameters::size() const noexcept
{
	return params_.size();
}